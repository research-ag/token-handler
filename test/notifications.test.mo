...
do {
  ...
  mock_ledger.balance.lock("INCREASE_FEE_WHILE_NOTIFY_IS_UNDERWAY_SCENARIO_1");
  let f1 = async { await* handler.notify(user1) };
  mock_ledger.fee.set(10); // fee 6 -> 10
  ...
  mock_ledger.balance.lock("INCREASE_FEE_WHILE_NOTIFY_IS_UNDERWAY_SCENARIO_2");
  let f2 = async { await* handler.notify(user1) }; // would return ?(5, _) at old fee
  mock_ledger.fee.set(15); // fee 10 -> 15
  ...
  mock_ledger.balance.lock("DECREASE_FEE_WHILE_NOTIFY_IS_UNDERWAY");
  let f3 = async { await* handler.notify(user1) };
  mock_ledger.fee.set(10); // fee 15 -> 10
  ...
  mock_ledger.balance.lock("CALL_MULTIPLE_NOTIFY_SIMULTANEOUSLY");
