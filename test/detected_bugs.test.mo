...
//   let (handler, _journal, _) = Util.createHandler(mock_ledger, false);
//
...
//   assert (await* TokenHandler.depositFromAllowance(handler, user1, user1_account, 10, null)) == #ok(10, 1);
...
//   assert (await* TokenHandler.withdrawFromCredit(handler, user1, account, 10, null)) == #ok(2, 10);
//
...
//   ignore (await* TokenHandler.notify(handler, user1));
// };
