...
do {
  ...
  assert (await* TokenHandler.notify(handler, user1)) == ?(20, 17);
  assert journal.hasEvents([
  ...
  assert (await* TokenHandler.withdrawFromCredit(handler, user1, account, 10, null)) == #err(#CallIcrc1LedgerError);
  ...
  assert (await* TokenHandler.withdrawFromCredit(handler, user1, account, 10, ?100)) == #err(#BadFee({ expected_fee = 3 }));
  ...
  assert (await* TokenHandler.withdrawFromPool(handler, account, 10, ?5)) == #err(#BadFee({ expected_fee = 1 }));
  ...
  assert (await* TokenHandler.withdrawFromPool(handler, account, 10, null)) == #err(#BadFee({ expected_fee = 3 }));
  ...
  assert (await* TokenHandler.withdrawFromPool(handler, account, 10, null)) == #ok(42, 7);
